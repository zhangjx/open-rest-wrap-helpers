...
const pageParams = (pagination, params) => {
  ...
  const limit = Math.max((+params.limit || +pagination.limit), 0);
  return {
    offset: Math.min(offset, pagination.maxOffset),
...
const findAllOpts = (Model, params, isAll) => {
  ...
  if (Model.rawAttributes.is_deleted && !params.showDeleted) {
    where.is_deleted = 'no';
  }
  ...
  if (includes) {
    _.each(includes, (x) => {
      ...
      if (x.model.rawAttributes.is_deleted && !params.showDeleted) {
        includeWhere.$or = [{ is_deleted: 'no' }];

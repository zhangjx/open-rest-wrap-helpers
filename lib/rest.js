...
module.exports = (restapi) => {
  const { Sequelize, errors } = restapi;
  ...
  const beforeAdd = (Model, cols, hook) => (req, res, next) => {
    ...
    const _save = (model) => {
      model.save().then((mod) => {
        req.hooks[hook] = mod;
        return next();
      }).catch(error => next(errors.IfError(error, Model.name)));
    };

    // 约定的 creator_id, 等于 req.user.id
    if (Model.rawAttributes.creator_id) attr.creator_id = req.user.id;
    // 约定的 client_ip, 等于 req.ip
    if (Model.rawAttributes.client_ip) attr.client_ip = req.ip;

    // 如果没有设置唯一属性，或者没有开启回收站
    if ((!Model.unique) || (!Model.rawAttributes.is_deleted)) {
    ...
    Model.findOne({ where }).then((model) => {
      ...
      if (model) {
        // 且资源曾经被删除
        if (model.is_deleted === 'yes') {
          _.extend(model, attr);
          // 恢复为正常状态
          model.is_deleted = 'no';
        } else {
          // 资源已经存在，重复了
          return next(errors.AlreadyExists(Model.name, Model.unique[0]));
        }
      } else {
        // 构建一个全新的资源
  ...
  const add = (Model, cols, hook, attachs) => {
    ...
    // 为了避免冲突导致，这里引入了随机字符串
    if (!hook) hook = `${Model.name}_${utils.randStr(10)}`;
  ...
  const remove = hook => (req, res, next) => {
    const model = req.hooks[hook];
    ((() => {
      // 资源如果有is_deleted 字段则修改is_deleted 为yes即可
      if (!model.is_deleted) return model.destroy();

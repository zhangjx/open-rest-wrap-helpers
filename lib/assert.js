...
module.exports = (restapi) => {
  ...
  const equal = (keyPath, obj, error) => (req, res, next) => {
  ...
  const notEqual = (keyPath, obj, error) => (req, res, next) => {
    const value = obj.fixed ? obj.fixed : _.get(req, obj.path);
    if (_.get(req, keyPath) !== value) return next();
    return next(error);
  };

  const _has = (obj1, obj2, has) => (req, res) => {
    ...
    if (_.isSet(value2)) {
      ...
      if (value2.includes(value1)) return has;
  ...
  const has = (obj1, obj2, error) => {
    const have = _has(obj1, obj2, true);
    return (req, res, next) => {
      if (have(req, res)) return next();
      return next(error);
    };
  };
  ...
  const notHas = (obj1, obj2, error) => {
    const notHave = _has(obj1, obj2, false);
    return (req, res, next) => {
      if (notHave(req, res)) return next();
      return next(error);
    };
  };

  // 检测是否存在
  const exists = (keyPath, error) => (req, res, next) => {
  ...
  const existsSchemas = [{
    ...
    type: Error,
    allowNull: false,
    defaultValue: restapi.errors.ResourceNotFound('Resource'),
    message: 'The error is called next when assert faild.',
  }];
